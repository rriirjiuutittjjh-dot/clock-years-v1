...
test("the build side resolves this workspace's app-env (sign-in on)", () => {
  // Sign-in was turned on per the auth skill (the VITE_AUTH_ENABLED key was
  // deleted from .grok/app-env.json), so an empty process env resolves on;
  // an explicit flag still wins either way.
