...
test("sign-in is on: app-env carries no off flag", () => {
  // \"Turning sign-in on\" (auth SKILL.md) deletes VITE_AUTH_ENABLED from
  // .grok/app-env.json; the predicate treats a missing flag as on. This pins
  // the on-state so re-adding `\"false\"` fails loudly instead of silently
  // disabling every login route.
...
test("the wrapper puts the project bin first on PATH", () => {
  const env = envWithLocalBin("/app", { PATH: "/usr/bin" });
  assert.equal(env.PATH, `/app/node_modules/.bin${process.platform === "win32" ? ";" : ":"}/usr/bin`);
});

test("the wrapper creates PATH when it is missing", () => {
  const env = envWithLocalBin("/app", {});
  assert.equal(env.PATH, "/app/node_modules/.bin");
});

test("the wrapped command runs with the app env applied", async () => {
  // Sign-in is on, so the file carries no VITE_AUTH_ENABLED and the wrapped
  // command observes the missing flag (which the auth predicate reads as on).
...
test("the CLI still runs when invoked through a symlinked path", async () => {
  ...
  // Sign-in is on: no VITE_AUTH_ENABLED in app-env, so the flag is missing.
  assert.equal(stdout, "undefined");
});
