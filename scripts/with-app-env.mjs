...
/**
 * Make local package binaries available even when npm did not inject
 * node_modules/.bin into PATH. This keeps the wrapper usable from CI, Docker,
 * service launchers, and direct `node scripts/with-app-env.mjs ...` calls
 * instead of silently falling back to an old preview/build.
 */
export function envWithLocalBin(root, env) {
  const pathKey = Object.keys(env).find((key) => key.toLowerCase() === "path") || "PATH";
  const currentPath = typeof env[pathKey] === "string" ? env[pathKey] : "";
  const localBin = join(root, "node_modules", ".bin");
  return {
    ...env,
    [pathKey]: currentPath ? `${localBin}${delimiter}${currentPath}` : localBin,
  };
}
...
function main(argv) {
  ...
  const env = envWithLocalBin(root, mergeAppEnv(readAppEnv(root), process.env));
