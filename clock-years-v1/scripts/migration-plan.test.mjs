...
test("sign-in is on: the globbed directory carries the auth copy", () => {
  // \"Turning sign-in on\" (auth SKILL.md) copies migrations/auth/0001_auth.sql
  // up into the globbed directory; the source stays in auth/ and the copy
  // must remain byte-identical (pinned by the copy test below).
